/* cherries.js — the gate for Do You Like Cherries? Productions
   ...
   Access (or a Worker) so the vault HTML is never served to outsiders.
   The check below just gates the reveal on the front end. */
(function(){
  'use strict';

  // demo password — swap for server-side auth before launch
  var PASSWORD = 'cherries';
  var KEY = 'ug_cherries_unlocked';
  ...
  function unlock(skipFocus){
    ...
    if(!skipFocus){
      var h = document.querySelector('.vhero h1');
